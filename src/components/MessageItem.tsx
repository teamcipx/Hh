...
const AudioPlayer = ({ src, duration }: { src: string; duration?: number | null }) => {
  ...
  return (
    <div className="bg-parchment-dark/60 border border-parchment-border/60 rounded-2xl p-3 flex items-center gap-3 shadow-inner max-w-sm">
      ...
      <button
        onClick={togglePlay}
        className="w-9 h-9 rounded-full bg-postal-red hover:bg-postal-darkred text-white flex items-center justify-center transition-all shadow active:scale-95 cursor-pointer shrink-0"
      >
        {isPlaying ? <Pause className="w-3.5 h-3.5 fill-white" /> : <Play className="w-3.5 h-3.5 fill-white ml-0.5" />}
      </button>

      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between text-[10px] font-mono text-slate-600 mb-1 font-semibold">
          <span className="flex items-center gap-1">
            <Mic className="w-3 h-3 text-postal-red animate-pulse" />
            <span>ভয়েস বার্তা (Voice Message)</span>
          </span>
        ...
        <div
          ...
          className="h-1.5 bg-white border border-parchment-border/30 rounded-full overflow-hidden cursor-pointer relative"
        >
          <div
...
export default function MessageItem({
  ...
  return (
    <div className={`flex flex-col ${isSelf ? "items-end" : "items-start"} mb-6 group relative`}>
      {/* Sender Label & Node details */}
      {!isSelf && (
        <span className="text-[11px] font-bold text-slate-600 mb-1.5 ml-2.5 font-mono uppercase tracking-wider">
      ...
      >
        ...
        <motion.div
          layout
          className={`rounded-2xl p-4 sm:p-5 relative overflow-hidden transition-all duration-300 border shadow-md ${
            isSelf
              ? showEncrypted
                ? "bg-slate-900 border-slate-700 text-slate-300 font-mono"
                : message.sender === "Saluk"
                ? "bg-[#faf5eb] text-slate-850 border-amber-200/90 shadow-amber-900/5 hover:shadow-lg"
                : "bg-[#f5fbf7] text-slate-850 border-emerald-200/90 shadow-emerald-900/5 hover:shadow-lg"
              : showEncrypted
              ? "bg-slate-900 border-slate-800 text-emerald-500 font-mono"
              : "bg-white text-slate-900 border-parchment-border/90 shadow-md hover:shadow-lg"
          }`}
        >
          {/* Authentic retro postage stamp in top corner */}
          {!showEncrypted && (
            <div className="absolute top-2.5 right-2.5 w-11 h-13 bg-amber-100 border-2 border-dashed border-amber-300 rounded flex flex-col items-center justify-between p-0.5 select-none pointer-events-none shadow-sm">
              {/* Stamp Crest details */}
              <div className="text-[6px] font-serif font-extrabold text-amber-800/80 tracking-tighter">DAKGHOR</div>
              <div className="w-6 h-6 rounded bg-amber-200/80 flex items-center justify-center border border-amber-300/40">
                {/* 19s theme label inside postage stamp */}
                <span className="text-[9px] font-serif font-black text-amber-900/80 rotate-12">19s</span>
              </div>
              <div className="text-[6px] font-mono font-bold text-amber-800/80">POST 10Tk</div>
              {/* Overlapping wavy circular post office cancel ink lines */}
              <div className="absolute inset-0 border border-transparent flex items-center justify-center opacity-30">
                <div className="w-9 h-9 rounded-full border border-dashed border-slate-900 flex items-center justify-center">
                  <span className="text-[4px] font-bold text-slate-900 rotate-45 tracking-widest">CANCELLED</span>
                </div>
              </div>
            </div>
          )}
          ...
          <AnimatePresence mode="wait">
            {showEncrypted ? (
              <motion.div
                ...
                exit={{ opacity: 0 }}
                className="font-mono text-[11px] leading-relaxed break-all select-all flex flex-col gap-2 pt-2 pr-10"
              >
                <div className="flex items-center gap-1.5 text-[9px] uppercase tracking-widest text-slate-400 border-b border-slate-800 pb-1.5 mb-1 font-bold">
                  <Lock className="w-3.5 h-3.5 text-postal-red" />
              ...
              >
                ...
                {message.text && (
                  <p className="leading-relaxed whitespace-pre-wrap font-serif text-[15px] text-slate-850 tracking-wide font-medium">
                    {message.text}
                  </p>
                )}
                ...
                {message.imageUrl && (
                  <div className="relative mt-3 rounded-xl overflow-hidden group/image border border-parchment-border bg-slate-100 shadow-inner">
                    <img
                      src={message.imageUrl}
                      alt={message.fileName || "Shared attachment"}
                      referrerPolicy="no-referrer"
                      className="max-h-56 sm:max-h-64 w-full object-cover cursor-pointer hover:scale-[1.01] transition-transform duration-300"
                    ...
                    <div className="absolute inset-0 bg-slate-950/40 opacity-0 group-hover/image:opacity-100 transition-opacity flex items-center justify-center">
                      <button
                        onClick={() => onImageClick(message.imageUrl!)}
                        className="p-2.5 bg-white rounded-full text-slate-800 shadow-md hover:bg-parchment-dark hover:scale-105 active:scale-95 transition-all cursor-pointer border border-parchment-border"
